...
docker run --name gitlab -it --rm [OPTIONS] \
    docker/gitlab app:rake gitlab:backup:create
```
## Restoring Backups
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    docker/gitlab app:rake gitlab:backup:restore
```
...
```bash
docker run --name gitlab -it --rm [OPTIONS] \
    docker/gitlab app:rake gitlab:backup:restore BACKUP=1417624827
```
